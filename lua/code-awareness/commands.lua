...
function M.test_highlights()
  ...
  vim.defer_fn(function()
    ...
    if ns_id then
      ...
      -- Show detailed info
      local msg = string.format('[code-awareness] Test highlights: %d extmarks, highlight: %s', 
        #extmarks, vim.inspect(hl_def))
      ...
      if #extmarks == 0 then
        ...
        vim.notify('[code-awareness] Extmarks created. If highlights are not visible, check highlight group and color.', vim.log.levels.INFO)
      end
    else
