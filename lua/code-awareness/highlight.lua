  ...
  local function apply()
    ...
    for _, line_nr in ipairs(line_numbers) do
      if type(line_nr) == 'number' and line_nr > 0 and line_nr <= line_count then
        ...
        util.log.debug(string.format('Skipping invalid line number: %s (type: %s, buffer lines: %d)', 
          tostring(line_nr), type(line_nr), line_count))
      end
    end
    ...
    if applied_count > 0 then
      ...
      for i = 1, math.min(3, #extmarks) do
        ...
        util.log.debug(string.format('Extmark %d: line=%d, col=%d, details=%s', 
          i, extmark[2], extmark[3], vim.inspect(extmark[4])))
      end
    end
  end
